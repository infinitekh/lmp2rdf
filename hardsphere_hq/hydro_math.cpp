...
const ihf ihf_zero = { .calculated =1, .x =0., .x2=0, .x3=0,
	.invx3=-10000,.cosx = 1., .cosx_x = 0,
...
void Hydrodynamic_Function::alloc_mem() {
	...
	ssf_all = (double *) malloc( sizeof(double) *kn );
 ...
 */
	...
	double SSF_part = 4.*M_PI*rho0*int_dr;
	...
	for (int xi = 1; xi < xn; xi++) {
		...
		mf_all[xi].part_s1 = SSF_part * h000[xi] * r2;
...
int Hydrodynamic_Function::run()
{
	...
	for (int ki = 1; ki < kn; ki++) {
		...
		for (int xi = 1; xi < xn; xi++) {
			...
			ssf_part += mf_all[xi].part_s1 * s1;


			
		}
		hf_all[ki] = D_s+part1+part2;
		ssf_all[ki] = 1. + ssf_part;;
	}
	printf("%s() at %s::%05d\n", __FUNCTION__, __FILE__, __LINE__);
	return 0;
}
